<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <x-text-input id="specialization" name="specialization" type="text" class="mt-1 block w-full" :value="old('specialization', $user->specialization)" required autofocus autocomplete="specialization" />
            <x-input-error class="mt-2" :messages="$errors->get('specialization')" />
        </div>

        <div>
            <x-input-label for="intro" :value="__('Intro')" />
            <x-text-input id="intro" name="intro" type="text" class="mt-1 block w-full" :value="old('intro', $user->intro)" required autofocus autocomplete="intro" />
            <x-input-error class="mt-2" :messages="$errors->get('intro')" />
        </div>

        <div>
            <x-input-label for="years_of_experience" :value="__('Years Of Experience')" />
            <x-text-input id="years_of_experience" name="years_of_experience" type="number" class="mt-1 block w-full" :value="old('years_of_experience', $user->years_of_experience)" required autofocus autocomplete="years_of_experience" />
            <x-input-error class="mt-2" :messages="$errors->get('years_of_experience')" />
        </div>

        <div>
            <x-input-label for="date_of_birth" :value="__('Location')" />
